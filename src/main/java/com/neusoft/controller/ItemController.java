...
//	private static final String rootPath=ResourceBundle.getBundle(baseName, locale).getBundle("db").getString("file.rootpath");
	@Value("${file.rootpath}")
	...
	* @throws
	 ...
	 * @return
	 */
	@RequestMapping(value = "/shoplist_zk")
	public String shoplistZK(Item item,String condition, Model model, HttpServletRequest request, HttpServletResponse response) {
		...
		String sql="SELECT * FROM item WHERE 1=1 and isDelete =0 and zk is not null order by zk asc ";
		Pager<Item> pagers = itemService.findBySqlRerturnEntity(sql);
		model.addAttribute("pagers", pagers);
		model.addAttribute("obj", item);
		model.addAttribute("condition", condition);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/shoplist_rx")
	public String shoplistRX(Item item,String condition, Model model, HttpServletRequest request, HttpServletResponse response) {
		...
		String sql="SELECT * FROM item WHERE 1=1 and isDelete =0 order by gmNum desc";
		Pager<Item> pagers = itemService.findBySqlRerturnEntity(sql);
		model.addAttribute("pagers", pagers);
		model.addAttribute("obj", item);
		model.addAttribute("condition", condition);
		...
		if (files.length > 0){
			...
			for (int s = 0;s < files.length;s++){
				 System.out.println("开始");  
		          ...
		          String path=rootPath+File.separator+n+files[s].getOriginalFilename();
		          System.out.println("===================================================");
	...
	public String exUpdate(Item item, Model model, @RequestParam("file") CommonsMultipartFile[] files, HttpServletRequest request, HttpServletResponse response) throws IllegalStateException, IOException {
		...
		if (files.length > 0){
			 // 0 1 2 3 4
			for (int s = 0;s < files.length;s++){
				CommonsMultipartFile commonsMultipartFile = files[s];
				if(commonsMultipartFile==null||commonsMultipartFile.isEmpty()){
					logger.debug("当前文件对象为空！");
					continue;
				}
		          long  startTime=System.currentTimeMillis();
		          System.out.println("fileName："+commonsMultipartFile.getOriginalFilename());
		          String n = UUIDUtils.create();
		          String path=rootPath+File.separator+n+commonsMultipartFile.getOriginalFilename();
		          System.out.println("===================================================");
		           System.out.println(path);
		          File newFile=new File(path);
		          //通过CommonsMultipartFile的方法直接写文件（注意这个时候）
		         ...
		         if (s == 0){
		        	 item.setUrl1("\\upload\\"+n+commonsMultipartFile.getOriginalFilename());
		         }
		         if (s == 1){
		        	 item.setUrl2("\\upload\\"+n+commonsMultipartFile.getOriginalFilename());
		         }
		         if (s == 2){
		        	 item.setUrl3("\\upload\\"+n+commonsMultipartFile.getOriginalFilename());
		         }
		         if (s == 3){
		        	 item.setUrl4("\\upload\\"+n+commonsMultipartFile.getOriginalFilename());
		         }
		         if (s == 4){
		        	 item.setUrl5("\\upload\\"+n+commonsMultipartFile.getOriginalFilename());
		         }
		    	}
			}
